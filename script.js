...
document.addEventListener('DOMContentLoaded', () => {
  ...
  style.textContent = '.img-ref-badge{position:absolute;top:8px;left:8px;background:#C6BEB5;color:#000;font-family:monospace;font-size:14px;font-weight:900;line-height:1;padding:5px 10px;z-index:9999;pointer-events:none;box-shadow:0 2px 8px rgba(0,0,0,.7);}';
  document.head.appendChild(style);
  let n = 1;
  document.querySelectorAll('img').forEach(img => {
    if (img.closest('.navbar') || img.closest('.footer') || img.closest('.welcome-modal')) return;
